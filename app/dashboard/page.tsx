...
function AnimatedCheck({ color = "#00e676" }: { color?: string }) {
  return (
...
function DashboardContent() {
  ...
  const { confirmPlan } = useUserPlan();

  // upgraded=pro, upgraded=elite, or upgraded=true (legacy)
  const upgradedParam = params.get("upgraded") ?? "";
  const sessionId     = params.get("session_id") ?? null;

  // Treat "true" as "pro" for backwards compat with old sessions
  const upgradedPlan  = upgradedParam === "true" ? "pro" : upgradedParam;
  const isUpgrade     = upgradedPlan === "pro" || upgradedPlan === "elite";
  const isEliteUpgrade = upgradedPlan === "elite";

  const [ready,  setReady]  = useState(false);
  const [status, setStatus] = useState(`Activating your ${isEliteUpgrade ? "Elite" : "Pro"} plan…`);
  const [activatedPlan, setActivatedPlan] = useState<string>(upgradedPlan || "pro");

  useEffect(() => {
    if (!isUpgrade) { router.replace("/"); return; }

    const clientId = localStorage.getItem("ciq_client_id");

    async function activate() {
      // Direct activation: retrieve Stripe session and upgrade profile immediately
      if (sessionId && clientId) {
        try {
          setStatus(`Activating your ${isEliteUpgrade ? "Elite" : "Pro"} plan…`);
      ...
      const poll = async () => {
        ...
        if (attempts < 8) {
          ...
          // Last resort — payment completed, activate with the plan from the URL param
          console.warn("[dashboard] could not confirm plan in Supabase after 8 attempts");
          const fallbackPlan = upgradedPlan || "pro";
          setActivatedPlan(fallbackPlan);
          confirmPlan(fallbackPlan);
          setReady(true);
        }
      };
      setTimeout(poll, 800);
  ...
  const accentColor    = isEliteUpgrade || activatedPlan === "elite" ? "#a78bfa" : "#00e676";
  const planLabel      = activatedPlan === "elite" ? "ELITE" : "PRO";
  const welcomeHeading = activatedPlan === "elite" ? "WELCOME TO ELITE" : "WELCOME TO PRO";
  const welcomeSubtext = activatedPlan === "elite"
    ? "Full automation unlocked. ChartIQ AI Signal System, MT4/MT5 Expert Advisor, and unlimited everything."
    : "Unlimited analyses unlocked. No daily limits, ever.";
  const welcomeFeatures = activatedPlan === "elite"
    ? ["Unlimited chart analyses", "MT4/MT5 Expert Advisor auto-trading", "ChartIQ AI Signal System", "Full automated signal system", "Priority support"]
    : ["Unlimited chart analyses", "No daily limits", "Full risk analysis", "Priority support"];
  ...
  return (
    <div className="min-h-screen bg-[#080a10] text-white flex flex-col items-center justify-center px-6">
      <motion.div
        ...
        style={{
          background: activatedPlan === "elite" ? "#0a0814" : "#080c0a",
          border: `1px solid ${accentColor}47`,
          boxShadow: `0 0 80px ${accentColor}14, 0 24px 64px rgba(0,0,0,0.6)`,
        }}
      >
        {/* Animated check */}
          ...
          {Array.from({ length: 6 }, (_, i) => {
            ...
            return (
              <motion.div key={i}
                ...
                style={{ width: 6, height: 6, background: accentColor, marginLeft: -3, marginTop: -3 }}
        ...
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full mb-4"
          style={{ background: `${accentColor}1f`, border: `1px solid ${accentColor}40` }}>
          <span className="text-xs font-bold tracking-widest font-dm-mono" style={{ color: accentColor }}>{planLabel}</span>
        </div>
          ...
          {welcomeFeatures.map((f) => (
            <li key={f} className="flex items-center gap-2.5 text-sm text-[#d1d5db]">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="flex-shrink-0">
                <path d="M2.5 7l3 3L11.5 3.5" stroke={accentColor} strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              {f}
            </li>
          ))}
        ...
        <Link href="/"
          ...
          style={{
            background: activatedPlan === "elite" ? "rgba(167,139,250,0.15)" : accentColor,
            color: activatedPlan === "elite" ? accentColor : "#080c0a",
            border: activatedPlan === "elite" ? `1px solid ${accentColor}50` : "none",
            boxShadow: `0 0 28px ${accentColor}4d`,
          }}>
          Start Analyzing →
        </Link>
